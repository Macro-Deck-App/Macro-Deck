...
Authoring a view (covered under [Concepts](/ui/concepts/ui-model/)) is half the job; the other half
...
| --- | --- | --- | --- |
| `config` | An integration's config flow, a configured action instance, a folder's view, or a widget | exclusive | [Serving a configuration view](/ui/views/configuration/), [Configuring a widget](/ui/views/widget-configuration/) |
| `widget` | A deck widget | shared | [Deck widget views](/ui/views/widget/) |
| `folder` | A folder that selected a custom view | shared | [Folder views](/ui/views/folder-views/) |
| `dialog` | A modal an action opened | exclusive | [Modal views](/ui/views/modal/) |
| `developer-preview` | A `[UiPreview]` scenario, opened from Developer Tools | exclusive | [Developer preview](/ui/views/developer-preview/) |
...
[Serving a view](/ui/views/sessions/) for the full session lifecycle shared by every kind.
...
slot - and falls back to the declared fields. See [Serving a configuration view](/ui/views/configuration/)
