...
producer chose, not the theme - see [Colours and text](/ui/concepts/theming/).
...
can complete a dialog, `answer`. See [Sizing](/ui/concepts/sizing/) for how `mainSize` and `fill`
resolve, and [Events](/ui/concepts/events/) for how a container answers a dialog.
...
font - see [Sizing](/ui/concepts/sizing/) for the exact rule and why a text needs its own `mainSize`
