...
| `double-press` | `UiComponentEvents.DoublePress` | `ui.slider`, `ui.button` | Two taps completed in quick succession. On a slider, each tap without a drag, sent after the second tap's `change`, never instead of it - on its own on a relative slider, whose taps send no `change`. On a button, or any node declaring a press name, it replaces both taps' `press`, which is held for 400 ms after each tap while it is declared. See [Button](/ui/components/button/#reader-behaviour). | None |
