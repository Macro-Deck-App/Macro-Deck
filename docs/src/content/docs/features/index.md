...
For the state labels themselves, reach for the `States.*` family in `MacroDeckStrings` - `On`, `Off`, `Muted`, `Visible`, `Recording`, `Unavailable` and similar - rather than declaring your own copies; see [Reuse `MacroDeckStrings`](/features/localization/#reuse-macrodeckstrings-instead-of-duplicating-common-strings).
...
The provider overrides the rendered icon at render time only: the widget's own configured icon (per state, where applicable) is never rewritten, and reappears the moment the provider is removed or disabled. Set Icon fails on a widget with an active icon provider for the same reason it fails on a state-controlled one; the icon property alone is dropped from a mixed patch, so the rest still applies. See [Capability parity](/reference/capability-parity/) for the poll/push story a plugin sees, and [Device providers](/features/devices/) for how a provider-controlled icon reaches a physical device.
...
See [Variables](/features/variables/) for materialization, attributes, the write capability, discovery,
...
[Device providers](/features/devices/).
...
layout through `DeviceDescriptor.LayoutReference`. See [Layout providers](/features/layouts/).
...
qualified id and its own data for it. See [Widget types](/ui/views/widget-types/).
...
For the session lifecycle, the limits and what a limit trip looks like from a provider, see [Serving a view](/ui/views/sessions/).
...
For exact wire differences between in-process integrations and plugins, see [Capability parity](/reference/capability-parity/). For a complete working setup, see [Contributing an integration](https://github.com/Macro-Deck-App/Macro-Deck/blob/main/engineering/development/contributing-integrations.md).
