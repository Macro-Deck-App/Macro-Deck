...
| Group | Permissions |
| --- | --- |
| Host | `host:variables`, `host:user-variables`, `host:config`, `host:deck`, `host:scripts`, `host:widgets`, `host:notifications`, `host:action-interactions`, `host:devices`, `host:variable-values`, `host:layouts`, `host:folder-views`, `host:widget-types`, `host:event-bindings`, `host:screensavers`, `host:adb`, `host:messaging` |
...
issuer certificate signed the certificate; no detached signature file. None of these four files is listed
in `files[]`. Exact shape:
