...
| Capability and host API catalogues | `device-provider`/`devices`, `layout-provider`/`layouts`, `folder-view-provider`/`folder-views`, `widget-type-provider`/`widget-types`, `event-bindings` | Names stay; see [capability operations](/reference/protocol/#capability-operations). |
