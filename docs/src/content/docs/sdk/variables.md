...
- The other results are `NotWritable`, `NotFound`, `Unavailable` (can write, just not right now - e.g.
  ...
  a slider that silently does nothing, and conformance check [MDC0314](/sdk/conformance/) fails it.
...
`MacroDeckTestHost` drives all six operations; see [conformance](/sdk/conformance/) for MDC0311, MDC0314
and MDC0315.
