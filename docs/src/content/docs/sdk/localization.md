...
`manifest.json` - the same identity [MDP1001](/sdk/analyzers/#mdp1001) already validates. The
...
- [SDK reference](/sdk/) - the `MacroDeck.Localization` package.
- [Analyzers](/sdk/analyzers/) - the full MDLOC table alongside every other diagnostic.
...
- [Theming](/sdk/ui/concepts/theming/) - `UiText` and where a `LocalizedString` is accepted on a view.
