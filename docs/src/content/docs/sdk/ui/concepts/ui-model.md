...
it was computed from rather than blindly applied. See [Patches](/sdk/ui/reference/patches/) for the
...
- **`MacroDeck.Ui`** - the declarative C# DSL and fine-grained reactive runtime that produces and updates
  ...
  [State and bindings](/sdk/ui/concepts/state-and-bindings/) and
  [Reactive updates](/sdk/ui/concepts/reactive-updates/).
- **`MacroDeck.Ui.Testing`** - a headless renderer/test host for views written against either of the
  above. See [Custom views](/sdk/ui/views/custom/) for a worked test.

All three are public NuGet contracts; see [Compatibility](/sdk/ui/reference/compatibility/).
...
folder). See [Serving a view](/sdk/ui/views/sessions/) for where this fits in a session's lifecycle, and
[Compatibility](/sdk/ui/reference/compatibility/) for why the model's major version is 3.
