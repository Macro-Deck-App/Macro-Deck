...
None of these carry a payload beyond what the table describes. See the [component reference](/sdk/ui/components/)
for the geometry and semantics each name is paired with on the component that declares it, and
[Modal views](/sdk/ui/views/modal/) for `modal.complete`, the one event a tree emits that is not a
