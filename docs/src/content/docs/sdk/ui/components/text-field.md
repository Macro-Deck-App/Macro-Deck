...
`placeholder`, like any prompt, is written for the reader - see [Colours and text](/sdk/ui/concepts/theming/).
...
state write on the producer. See [Events](/sdk/ui/concepts/events/) for the shared interaction model.

## Layout behaviour

A leaf: `mainSize` or `fill` if declared on its parent stack, otherwise its content extent. `size` is the
font size, not a main-axis extent - see [Sizing](/sdk/ui/concepts/sizing/).
