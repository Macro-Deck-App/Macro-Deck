...
`ui.transform` draws its children the way [`ui.layer`](/sdk/ui/components/stack-and-layer/) does - every
...
unrotated axes. `zoom`, `offsetX` and `offsetY` are the same keys a [button](/sdk/ui/components/button/)
