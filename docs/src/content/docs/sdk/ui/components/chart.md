...
`ui.range-bar`'s `startColor` makes. See [Colours and text](/sdk/ui/concepts/theming/). Values outside
...
otherwise its content extent. See [Sizing](/sdk/ui/concepts/sizing/). Reserve a live numeric readout's
width beside a chart with `ui.text`'s `digits` property, or the readout moves the chart every time its
value gains or loses a digit - see [Text](/sdk/ui/components/text/).
