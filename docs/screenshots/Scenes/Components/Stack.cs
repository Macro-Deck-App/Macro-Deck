...
internal static partial class Scenes
{
		...
		[
			...
			new UiStack
			{
				...
				[
					new UiTextRun
					{
						Key = "title", Text = title, Size = UiSize.FromBasis(0.038),
						Weight = UiComponentTextWeights.Medium
					},
					new UiTextRun
					{
						Key = "subtitle", Text = subtitle, Size = UiSize.FromBasis(0.032),
	...
	private static IEnumerable<Scene> StackScenes()
	{
				...
				[
					new UiStack
					{
						...
						[
							...
							new UiTextRun
							{
								Key = "temp", Text = "21°", Size = 0.12, MainSize = 0.3,
		...
		yield return Tile("stack-space-between",
			new UiStack
			{
				Key = "tile",
				Justify = UiComponentJustify.Center,
				...
				[
					new UiStack
					{
						...
						[
							...
							new UiTextRun
								{ Key = "temp", Text = "21°", Size = 0.24, Weight = UiComponentTextWeights.Bold },
						],
					},
				],
			},
		...
		yield return Tile("stack-baseline",
			Centered("tile",
				new UiStack
				{
					Key = "reading",
	...
	private static UiLayer HistoryGraph(string title, string subtitle, string value) => new()
	{
		...
		[
			new UiChart
			{
				Key = "chart", Points = SamplePoints, PlotTop = 0.66, Thickness = UiSize.Capped(2d / UiLength.Cell, 2)
			},
			new UiStack
			{
				Key = "labels",
			...
			new UiStack
			{
				...
				[
					new UiTextRun
					{
						Key = "reading", Text = value, Size = 0.22, Weight = UiComponentTextWeights.Bold,
						Align = UiComponentAlignments.Center
					}
				],
			},
	...
	private static UiValue<IReadOnlyList<double>> SamplePoints => UiValue.Of<IReadOnlyList<double>>([
		0.32, 0.35, 0.3, 0.42, 0.55, 0.48, 0.5, 0.62, 0.58, 0.44, 0.4, 0.47, 0.6, 0.72, 0.66, 0.52, 0.49, 0.56, 0.63,
		0.58
	]);
}
