...
entry and the token is refused rather than ignored. `RegisterOrReuse` is the only path that creates a
device, and it tracks the new device with the guard.
...
**Access token lifetimes are per scope.** Admin stays at 15 minutes, and so does a session without a
device. Client scope - deck devices and the companion - moves to 60 days
(`AuthDefaults.AccessTokenLifetimeFor`).
...
- A client-scope token outlives `setTimeout`'s 2^31-1 ms ceiling (about 24.8 days), which silently fires
  at once rather than late. Both clients arm the refresh timer in chunks and re-arm
  (`MAX_REFRESH_DELAY_MS`).
- A password reset now also records the revocation per device (`EndAllSessions`). `AccessTokenCutoff`
  lives only in memory, so a restart used to bring back every access token it had refused; at 60 days
  that would have been a real hole rather than a 15 minute one.
...
- The guard is authoritative for device claims, so any future path that creates a device without going
  through `RegisterOrReuse` would have its tokens refused until the next restart reseeds the guard.
