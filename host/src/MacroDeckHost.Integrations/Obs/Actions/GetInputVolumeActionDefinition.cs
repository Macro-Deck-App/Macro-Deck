...
internal sealed class GetInputVolumeActionDefinition : IDynamicOptionsActionDefinition
{
	...
	public GetInputVolumeActionDefinition(Func<ObsConnection?> resolver, VariableApiAccessor variables)
		: this(ObsTargetResolver.Legacy(resolver), variables)
	{
	}

	public GetInputVolumeActionDefinition(ObsTargetResolver resolver, VariableApiAccessor variables)
	{
		_resolver = resolver;
		_variables = variables;
	}

	public string Id => "get-input-volume";

	public LocalizedText Name => AppStrings.Integrations.Obs.Actions.GetInputVolume.Name();

	public LocalizedText Description => AppStrings.Integrations.Obs.Actions.GetInputVolume.Description();
	...
	private sealed class Executor : IActionExecutor
	{
		private static readonly ILogger _logger =
			IntegrationLog.For<GetInputVolumeActionDefinition>(ObsIntegration.IntegrationId);
		...
		public async Task<ActionResult> ExecuteAsync(ActionExecutionContext context)
		{
			...
			{
				_logger.Warning("OBS get-volume action skipped: no input selected");
			...
			{
				_logger.Warning("OBS get-volume action skipped: no target variable");
				return ActionResult.Failed(ActionErrorCodes.InvalidParameter,
					AppStrings.Integrations.Obs.Errors.NoTargetVariable());
			}

			var percent = await connection.GetInputVolumePercentAsync(input);
			if (percent is null)
			{
				_logger.Warning("OBS get-volume action skipped: volume unavailable for '{Input}'", input);
