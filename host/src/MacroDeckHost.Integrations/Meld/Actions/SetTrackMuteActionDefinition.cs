...
internal sealed class SetTrackMuteActionDefinition : IDynamicOptionsActionDefinition, IStateProviderActionDefinition
{
	private readonly Func<MeldConnection?> _resolver;

	public SetTrackMuteActionDefinition(Func<MeldConnection?> resolver)
	{
		_resolver = resolver;
	}

	public string Id => "set-track-mute";

	public LocalizedText Name => AppStrings.Integrations.Meld.Actions.SetTrackMute.Name();

	public LocalizedText Description => AppStrings.Integrations.Meld.Actions.SetTrackMute.Description();
	...
	[
		...
		ActionParameter.Choice(MeldActionParameters.Mode,
			options: MeldActionParameters.TrackMuteModes,
	...
	public Task<ActionStateSnapshot?> GetActionStateAsync(
		...
		=> Task.FromResult(MeldActionStates.Snapshot(_resolver(),
			...
			track => track.Name,
			MeldTargetResolver.CurrentMuted));
	...
	private sealed class Executor : IActionExecutor
	{
		...
		public async Task<ActionResult> ExecuteAsync(ActionExecutionContext context)
		{
			...
			return await connection.SetTrackMutedAsync(target!.Id, desired, context.CancellationToken)
