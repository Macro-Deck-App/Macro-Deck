...
public class PluginWebSocketEndpointDispatchTests
{
		...
		CreateEndpoint(IEventBus? eventBus = null)
	{
		...
		var statePusher
			= new HostStatePusher(registry, new EmptyDeckNavigator(), new EmptyScriptApi(), new EmptyWidgetApi(), new MacroDeckHost.Application.Deck.DeckClientTracker(Serilog.Core.Logger.None), Serilog.Core.Logger.None);
