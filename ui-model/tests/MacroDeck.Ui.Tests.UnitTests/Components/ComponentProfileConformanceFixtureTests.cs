...
public class ComponentProfileConformanceFixtureTests
{
	...
	{
		var tree = JsonSerializer.Deserialize<UiTree>(ReadTree("conformance-gauge-tree.json"), UiCanonicalJson.Options)!;
		...
		Assert.Multiple(() =>
		{
			...
			Assert.That(nodes["conformance.needle"].Fallback?.Type, Is.EqualTo("ui.text"),
				"a reader without ui.transform must still have a reading to draw");
			Assert.That(nodes["conformance.identity"].Properties, Is.Empty,
