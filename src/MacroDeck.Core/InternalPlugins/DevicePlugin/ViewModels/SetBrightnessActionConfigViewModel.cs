...
public class SetBrightnessActionConfigViewModel : ISerializableConfigViewModel
{
    ...
    public void SetConfig()
    {
        _action.ConfigurationSummary
            = $"{(string.IsNullOrWhiteSpace(ClientId) ? LanguageManager.Strings.WhereExecuted : DeviceManager.GetKnownDevices().Find(x => x.ClientId.Equals(ClientId)).DisplayName)} -> {Math.Round(Brightness * 100)}%";
