...
public partial class SetBrightnessActionConfigView : ActionConfigControl
{
    ...
    private void LoadKnownDevices()
    {
        foreach (var macroDeckDevice in DeviceManager.GetKnownDevices()
            .FindAll(x => x.DeviceType == DeviceType.Android || x.DeviceType == DeviceType.iOS).ToArray())
        {
    ...
    public override bool OnActionSave()
    {
        ...
        if (radioFixedDevice.Checked && string.IsNullOrWhiteSpace(_viewModel.ClientId))
        {
            return false;
        }
    ...
    {
        if (!radioFixedDevice.Checked)
        {
            return;
        }

        var macroDeckDevice = DeviceManager.GetKnownDevices().Find(x => x.DisplayName.Equals(devicesList.Text));
        if (macroDeckDevice == null || !macroDeckDevice.Available)
        {
            return;
        }
