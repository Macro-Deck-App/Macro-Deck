...
public class VerticalTabControl : TabControl
{
	...
	protected override void OnPaint(PaintEventArgs e)
	{
		...
		g.DrawLine(new Pen(Color.FromArgb(45, 45, 45)),
		...
		{
			var buttonRectangle = new Rectangle(new Point(GetTabRect(i).Location.X - 2, GetTabRect(i).Location.Y - 2),
			...
			if (_notificationIndexes.Contains(i))
			{
				g.FillEllipse(Brushes.Red,
					buttonRectangle.X + buttonRectangle.Width - 22,
					buttonRectangle.Y + 6,
					10,
					10);
			}
			...
			if (ImageList != null)
			{
				try
				{
					...
					g.DrawImage(ImageList.Images[i],
						new Rectangle(buttonRectangle.Location.X + 8,
							buttonRectangle.Location.Y + 22 - 24 / 2,
							24,
							24));
