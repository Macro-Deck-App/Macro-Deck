...
public partial class ConditionItem : UserControl, IActionConditionItem
{
    ...
    private string[] boolSuggestions =
    {
        "True",
        "False"
    };

    private string[] stateSuggestions =
    {
        "On",
        "Off"
    };
    ...
    private void MoveDownClicked(object sender, EventArgs e)
    {
        ...
        {
            var currentIndex = ((ConditionAction)Action).Actions.IndexOf(action);
            if (currentIndex + 1 >= ((ConditionAction)Action).Actions.Count)
            {
                return;
            }
    ...
    private void EditClicked(object sender, EventArgs e)
    {
        ...
        {
            if (configurator.Action.CanConfigure && configurator.Action.Configuration.Length == 0)
            {
                return;
            }
    ...
    {
        if (((ConditionAction)Action).ConditionType == ConditionType.Template &&
    ...
    {
        ((ConditionAction)Action).ConditionMethod
            = (ConditionMethod)Enum.Parse(typeof(ConditionMethod), methodBox.Text);
        if (((ConditionAction)Action).ConditionType == ConditionType.Variable &&
            string.IsNullOrWhiteSpace(methodBox.Text))
        {
            try
            {
    ...
    private void Source_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (variable != null)
        {
            ...
            else if ((variable.Suggestions == null || variable.Suggestions.Length == 0) &&
                variable.Type.Equals(VariableType.Bool))
            {
                valueToCompare.SetAutoCompleteMode(AutoCompleteMode.Suggest);
                valueToCompare.SetAutoCompleteSource(AutoCompleteSource.CustomSource);
