...
public partial class SetupPage5 : UserControl
{
	...
	private void LoadAvailableIconPacks(bool autoInstall = false)
	{
		...
		try
		{
			...
			var jsonString = wc.DownloadString("https://macrodeck.org/files/packagemanager/iconpacks.php?target-api=" +
	...
	public List<JObject> GetObjectsToInstall()
	{
		...
		{
			if (initialSetupIconPackItem.GetType() != typeof(InitialSetupIconPackItem))
			{
				continue;
			}
	...
	{
		Task.Run(() => { LoadAvailableIconPacks(true); });
	}
}
