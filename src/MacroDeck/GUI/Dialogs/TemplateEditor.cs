...
public partial class TemplateEditor : DialogForm
{
    ...
    private readonly Regex operatorRegex = new(@$"\b(?x: {string.Join(" | ", TemplateManager.Operators)})\b",
        RegexOptions.Singleline | RegexOptions.Compiled);

    private readonly Regex functionRegex = new(@$"\b(?x: {string.Join(" | ", TemplateManager.Functions)})\b",
        RegexOptions.Singleline | RegexOptions.Compiled);

    private readonly Regex commandRegex = new(@$"\b(?x: {string.Join(" | ", TemplateManager.Commands)})\b",
        RegexOptions.Singleline | RegexOptions.Compiled);

    private readonly Regex specialRegex = new(@$"\b(?x: {string.Join(" | ", TemplateManager.Special)})\b",
        RegexOptions.Singleline | RegexOptions.Compiled);
    ...
    public TemplateEditor(string template = "")
    {
        ...
        variableRegex = new Regex(@$"\b(?x: {string.Join(" | ", variablesList)})\b",
            RegexOptions.Singleline | RegexOptions.Compiled);
    ...
    private void BtnVariables_Click(object sender, EventArgs e)
    {
        ...
        variablesContextMenu.Show(PointToScreen(new Point(((ButtonPrimary)sender).Bounds.Left,
            ((ButtonPrimary)sender).Bounds.Bottom)));
    }
    ...
    private void CheckTrimBlankLines_CheckedChanged(object sender, EventArgs e)
    {
        ...
        {
            Template = Template.Replace(TrimBlankNewLine, string.Empty)
                .Replace(TemplateManager.TemplateTrimBlank, string.Empty);
        }
    }
}
