...
public partial class ButtonEditor : DialogForm
{
	...
	private void UpdateLabel()
	{
		try
		{
			var buttonLabel = radioButtonOff.Checked && !radioButtonOn.Checked
				? actionButtonEdited.LabelOff
				: actionButtonEdited.LabelOn;
			...
			Task.Run(() =>
			{
				...
				labelBitmap = (Bitmap)LabelGenerator.GetLabel(labelBitmap,
					labelText,
					buttonLabel.LabelPosition,
					new Font(buttonLabel.FontFamily, buttonLabel.Size),
					buttonLabel.LabelColor,
					Color.Black,
					new SizeF(2.0F, 2.0F));
	...
	public void RefreshIcon()
	{
		try
		{
			var iconString = radioButtonOff.Checked && !radioButtonOn.Checked
				? actionButtonEdited.IconOff
				: actionButtonEdited.IconOn;
			...
			var backColor = radioButtonOff.Checked && !radioButtonOn.Checked
				? actionButtonEdited.BackColorOff
				: actionButtonEdited.BackColorOn;
			btnBackColor.BackColor = backColor;
			btnBackColor.HoverColor = backColor;
			btnPreview.BackColor = backColor;

			btnPreview.ShowGIFIndicator = btnPreview.BackgroundImage != null &&
				btnPreview.BackgroundImage.RawFormat.ToString().ToLower() == "gif";
		}
		catch (Exception ex)
		{
			MacroDeckLogger.Error(GetType(),
	...
	private void Apply()
	{
		...
		foreach (var actionButton in folder.ActionButtons.FindAll(actionButton =>
				actionButton.Position_Y == this.actionButton.Position_Y &&
				actionButton.Position_X == this.actionButton.Position_X).ToArray())
		{
	...
	private void OpenIconSelector()
	{
		...
		if (iconSelector.ShowDialog() == DialogResult.OK)
		{
			if (iconSelector.SelectedIconPack != null && iconSelector.SelectedIcon != null)
			{
				...
				{
					actionButtonEdited.IconOff
						= iconSelector.SelectedIconPack.Name + "." + iconSelector.SelectedIcon.IconId;
				}
				else
				{
					actionButtonEdited.IconOn
	...
	{
		actionButtonEdited = JsonConvert.DeserializeObject<ActionButton.ActionButton>(
			JsonConvert.SerializeObject(actionButton, jsonSerializerSettings),
			jsonSerializerSettings); // Make a copy of the current action button
	...
	private void BtnBackColor_Click(object sender, EventArgs e)
	{
		...
		{
			Color = radioButtonOff.Checked && !radioButtonOn.Checked
				? actionButtonEdited.BackColorOff
				: actionButtonEdited.BackColorOn,
			FullOpen = true,
	...
	private void BtnAddVariable_Click(object sender, EventArgs e)
	{
		...
		variablesContextMenu.Show(PointToScreen(new Point(((PictureButton)sender).Bounds.Left,
			((PictureButton)sender).Bounds.Bottom)));
	}
