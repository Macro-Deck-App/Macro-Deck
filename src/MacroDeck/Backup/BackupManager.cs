...
public class BackupManager
{
	...
	public static void CheckRestoreDirectory()
	{
		...
		if (restoreBackupInfo.RestoreConfig &&
			File.Exists(Path.Combine(restoreDirectory, Path.GetFileName(ApplicationPaths.MainConfigFilePath))))
		{
			try
			{
				File.Copy(Path.Combine(restoreDirectory, Path.GetFileName(ApplicationPaths.MainConfigFilePath)),
		...
		if (restoreBackupInfo.RestoreProfiles &&
			File.Exists(Path.Combine(restoreDirectory, Path.GetFileName(ApplicationPaths.ProfilesFilePath))))
		{
			try
			{
				File.Copy(Path.Combine(restoreDirectory, Path.GetFileName(ApplicationPaths.ProfilesFilePath)),
		...
		if (restoreBackupInfo.RestoreDevices &&
			File.Exists(Path.Combine(restoreDirectory, Path.GetFileName(ApplicationPaths.DevicesFilePath))))
		{
			try
			{
				File.Copy(Path.Combine(restoreDirectory, Path.GetFileName(ApplicationPaths.DevicesFilePath)),
		...
		if (restoreBackupInfo.RestoreVariables &&
			File.Exists(Path.Combine(restoreDirectory, Path.GetFileName(ApplicationPaths.VariablesFilePath))))
		{
			try
			{
				File.Copy(Path.Combine(restoreDirectory, Path.GetFileName(ApplicationPaths.VariablesFilePath)),
				...
				new DirectoryInfo(ApplicationPaths.PluginsDirectoryPath).Name)))
		{
			try
			{
				...
				DirectoryCopy.Copy(
					Path.Combine(restoreDirectory, new DirectoryInfo(ApplicationPaths.PluginsDirectoryPath).Name),
				...
				new DirectoryInfo(ApplicationPaths.PluginCredentialsPath).Name)))
		{
			try
			{
				...
				DirectoryCopy.Copy(
					Path.Combine(restoreDirectory, new DirectoryInfo(ApplicationPaths.PluginCredentialsPath).Name),
				...
				new DirectoryInfo(ApplicationPaths.IconPackDirectoryPath).Name)))
		{
			try
			{
				...
				DirectoryCopy.Copy(
					Path.Combine(restoreDirectory, new DirectoryInfo(ApplicationPaths.IconPackDirectoryPath).Name),
	...
	public static void CreateBackup()
	{
		...
		try
		{
			...
			File.Copy(ApplicationPaths.MainConfigFilePath,
				Path.Combine(tempBackupPath, Path.GetFileName(ApplicationPaths.MainConfigFilePath)));
			File.Copy(ApplicationPaths.ProfilesFilePath,
				Path.Combine(tempBackupPath, Path.GetFileName(ApplicationPaths.ProfilesFilePath)));
			File.Copy(ApplicationPaths.DevicesFilePath,
				Path.Combine(tempBackupPath, Path.GetFileName(ApplicationPaths.DevicesFilePath)));
			File.Copy(ApplicationPaths.VariablesFilePath,
				Path.Combine(tempBackupPath, Path.GetFileName(ApplicationPaths.VariablesFilePath)));
